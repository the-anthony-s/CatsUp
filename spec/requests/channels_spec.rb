...
RSpec.describe 'Channels API', type: :request do
  ...
  let!(:channels) { create_list(:channel, 10, user_id: user.id, recipient_id: recipient.id) }
  ...
  describe 'POST /channels' do
    ...
    let(:valid_attributes) {
      { name: 'Cats vs Dogs', user_id: user.id, recipient_id: recipient.id }
    }
  ...
  describe 'GET /channels/search' do
    ...
    context 'when the record exists' do
      ...
      it 'contains the channels' do
        expect(JSON.parse(response.body).first['name']).to eq(channel_name)
      end
    ...
    context 'when the params are uppercase' do
      let(:channel_name) { channels.first.name.upcase }
